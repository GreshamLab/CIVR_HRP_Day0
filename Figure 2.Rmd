...
```
###This code generates a list of differentially expressed genes, ranked by spearman correlation coefficient, with respect to baseline HAI
```{r}
target.genes <- degenes.edgeR$log2baseline
...
###This code generates a list of differentially expressed genes, ranked by spearman correlation coefficient, with respect to Seroconversion
```{r}
target.genes <- degenes.edgeR$log2sero
gsea.counts <- counts.subset[rownames(counts.subset) %in% target.genes,]
gsea.counts <- gsea.counts[,colSums(gsea.counts) != 0]
...
for (i in 1:length(ontlist)){
  ...
  simp <- clusterProfiler::simplify(gse, 
                                    semData = hsGO,
                                    cutoff = 0.5) #change to reduce terms
  simplist[[i]] <- simp
  tib.list[[i]] <- as_tibble(simp@result)
}
...
fig2$f <- gsea.diff.expression_all %>%
  ...
  labs(x = "Enrichment Score",
         y = "Description",
         title = "") +
  scale_size(name = "Gene Set Size", range = c(1, 20), limits = c(1, 170), breaks = c(5, 10, 50, 100, 150)) +
